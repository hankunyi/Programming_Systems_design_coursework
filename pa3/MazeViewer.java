...
public class MazeViewer {
   ...
   */
   private static MazeFrame readMazeFile(String fileName) throws IOException {
   	int mazeRow=0;	//the number of rows of Maze
   	int mazeCol=0;	//the number of columns of Maze
   	int startRow=0;	//the row number of entrance
   	int startCol=0;	//the column number of entrance
   	int endRow=0;	//the row number of exit
   	int endCol=0;	//the column number of exit
   	File inFile = new File(fileName);
   	Scanner in = new Scanner (inFile);
   	mazeRow = in.nextInt();
   	mazeCol = in.nextInt();
   	boolean[][] mazeConf = new boolean [mazeRow][mazeCol]; // the 2-dimension array that represents the maze
   	// Read maze char by char
   	for (int i=0; i<mazeRow; i++) {
   		String nextLine = in.next();
   		for (int j=0; j<mazeCol; j++) {
   			mazeConf[i][j] = (nextLine.charAt(j) == WALL_CHAR);
   		}
   	}
   	startRow = in.nextInt();
   	startCol = in.nextInt();
   	endRow = in.nextInt();
   	endCol = in.nextInt();	
      return new MazeFrame(mazeConf, new MazeCoord(startRow, startCol), new MazeCoord(endRow, endCol));
   }
